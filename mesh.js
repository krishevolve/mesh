...
class MeshAPIClient {
    ...
    async checkNodeStatus(token, uniqueId) {
        return this.retry(async () => {
            try {
    ...
    async linkNode(token, uniqueId, name, tgData) {
        return this.retry(async () => {
            try {
    ...
    async estimateRewards(token, uniqueId) {
        return this.retry(async () => {
            try {
    ...
    async startMining(token, uniqueId) {
        return this.retry(async () => {
            try {
    ...
    async claimRewards(token, uniqueId) {
        return this.retry(async () => {
            try {
    ...
    async claimMission(token, missionId) {
        return this.retry(async () => {
            try {
    ...
    async processAccount(authData, index) {
        ...
        console.log(`========== Tài khoản ${index + 1} | ${firstName.green} ==========`);
    
        try {
            this.log(`Đang đăng nhập tài khoản ${uniqueId}...`, 'info');
            const loginResult = await this.login(authData);
            this.log('Đăng nhập thành công!', 'success');

            const statusResult = await this.checkNodeStatus(loginResult.access_token, uniqueId);
            ...
            const missions = await this.checkMissions(loginResult.access_token);
            ...
            for (const missionId of missionIds) {
                ...
                if (mission && !mission.claimed_at) {
                    ...
                    try {
                        const claimResult = await this.claimMission(loginResult.access_token, missionId);
            ...
            const estimateResult = await this.estimateRewards(loginResult.access_token, uniqueId);
            
            if (estimateResult.needsStart) {
                this.log('Mining chưa được bắt đầu, đang khởi động...', 'info');
                const startResult = await this.startMining(loginResult.access_token, uniqueId);
                ...
                if (estimateResult.filled) {
                    ...
                    const claimResult = await this.claimRewards(loginResult.access_token, uniqueId);
